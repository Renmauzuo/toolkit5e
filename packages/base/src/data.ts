...
export const traits: Record<string, Trait> = {
  ...
  covenMagic:           { name: 'Coven Magic',           description: "While within 30 feet of at least two hag allies, {{description}} can cast one of the following spells, requiring no Material components, using Intelligence as the spellcasting ability (spell save DC {{trait:DC}}): Augury, Find Familiar, Identify, Locate Object, Scrying, or Unseen Servant. {{description}} must finish a Long Rest before using this trait to cast that spell again.", allowsSave: true, dcStat: 'int' },
  mimicry:              { name: 'Mimicry',               description: '{{description}} can mimic animal sounds and humanoid voices. A creature that hears the sounds can tell they are imitations with a successful DC {{trait:DC}} Wisdom (Insight) check.', allowsSave: true, dcStat: 'cha' },
  vileAppearance:       { name: 'Vile Appearance',       description: "Any Beast or Humanoid that starts its turn within 30 feet of {{description}} and can see {{pronoun:possessiveAdj}} true form must succeed on a DC {{trait:DC}} Wisdom saving throw or have the Frightened condition until the start of its next turn. On a successful save, the creature is immune to {{description}}'s Vile Appearance for 24 hours.", allowsSave: true, dcStat: 'wis' },
...
export const procs: Record<string, Trait> = {
  ...
  poisonBow:   { name: 'Sprite Poison', description: "The target must succeed on a DC {{trait:DC}} Constitution saving throw or become poisoned for 1 minute. If the saving throw result is 5 or lower, the poisoned target falls unconscious for the same duration, or until it takes damage or another creature takes an action to shake it awake.", allowsSave: true, dcStat: 'con' },
  charmBow:    { name: 'Enchanting', description: "The target has the Charmed condition until the start of {{description}}'s next turn." },
};
...
export const actions: Record<string, Trait> = {
  ...
  heartSight:      { name: 'Heart Sight', description: "{{description}} touches a creature and magically knows the creature's current emotional state. If the target fails a DC {{trait:DC}} Charisma saving throw, {{description}} also knows the creature's alignment. Celestials, fiends, and undead automatically fail the saving throw.", allowsSave: true, dcStat: 'cha' },
  ...
  blinkTeleport:   { name: 'Teleport (Recharge 4\u20136)', description: '{{description}} magically teleports, along with any equipment {{pronoun:subject}} is wearing or carrying, up to 40 feet to an unoccupied space {{pronoun:subject}} can see.' },
  centaurCharge:   { name: 'Trampling Charge (Recharge 5\u20136)', description: "{{description}} moves up to {{pronoun:possessiveAdj}} Speed without provoking Opportunity Attacks and can move through the spaces of Medium or smaller creatures. Each creature whose space {{description}} enters must succeed on a DC {{trait:DC}} Strength saving throw or take {{trait:damage}} bludgeoning damage and have the Prone condition.", allowsSave: true, dcStat: 'str', dealsDamage: true },
  deathGlare:      { name: 'Death Glare (Recharge 5\u20136)', description: "One frightened creature {{description}} can see within 30 feet must succeed on a DC {{trait:DC}} Wisdom saving throw or take {{trait:damage}} psychic damage. If the target has 20 hit points or fewer, it instead drops to 0 hit points.", allowsSave: true, dcStat: 'wis', dealsDamage: true },
  mockery:         { name: 'Mockery', description: "One creature {{description}} can see within 90 feet must succeed on a DC {{trait:DC}} Wisdom saving throw or take {{trait:damage}} psychic damage.", allowsSave: true, dcStat: 'cha', dealsDamage: true },
