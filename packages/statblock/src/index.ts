...
export function renderStatblock(sourceStats: Statblock, target: HTMLElement): void {
  ...
  if (stats.traits && Object.keys(stats.traits).length) {
    ...
    for (const traitName in stats.traits) {
      ...
      let resolvedName = replaceTokensInString(t.name, stats, t);
      if (t.recharge === 'short') resolvedName += ' (Recharges after a Short or Long Rest)';
      else if (t.recharge === 'long') resolvedName += ' (Recharges after a Long Rest)';
      const p = mkEl('p');
  ...
  if (hasAttacks) {
    ...
    if (stats.actions) {
      for (const actionName in stats.actions) {
        ...
        if (action.recharge === 'short') actionDisplayName += ' (Recharges after a Short or Long Rest)';
        else if (action.recharge === 'long') actionDisplayName += ' (Recharges after a Long Rest)';
        const p = mkEl('p');
        p.innerHTML = `<strong><em>${actionDisplayName}.</em></strong> ${action.text}`;
        attacksEl.appendChild(p);
      }
    }
  ...
  if (stats.bonusActions && Object.keys(stats.bonusActions).length) {
    ...
    for (const bonusActionName in stats.bonusActions) {
      ...
      let baDisplayName = ba.name;
      if (ba.recharge === 'short') baDisplayName += ' (Recharges after a Short or Long Rest)';
      else if (ba.recharge === 'long') baDisplayName += ' (Recharges after a Long Rest)';
      const p = mkEl('p');
      p.innerHTML = `<strong><em>${baDisplayName}.</em></strong> ${ba.text}`;
  ...
  if (stats.legendaryActions && Object.keys(stats.legendaryActions).length) {
    ...
    for (const actionName in stats.legendaryActions) {
      ...
      let laDisplayName = la.name;
      if (la.recharge === 'short') laDisplayName += ' (Recharges after a Short or Long Rest)';
      else if (la.recharge === 'long') laDisplayName += ' (Recharges after a Long Rest)';
      const p = mkEl('p');
      p.innerHTML = `<strong><em>${laDisplayName}.</em></strong> ${la.text}`;
